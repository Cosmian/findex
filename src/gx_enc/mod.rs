...
//!
//! Uses the Findex MM-Enc scheme in order to securely store a graph. This graph stores
//! `IndexedValue`s for `Tag`s. An `IndexedValue` can be either a `Pointer` to another `Tag` or a
//! `Data` containing a `Value`.
