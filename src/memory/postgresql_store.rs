...
impl MemoryADT for PostGresMemory {
    type Address = Vec<u8>;
    type Word = Vec<u8>;
    ...
    async fn batch_read(&self, a: Vec<Vec<u8>>) -> Result<Vec<Option<Vec<u8>>>, Self::Error> {
        ...
        let m = format!(
            "({})",
            (1..=addresses.len()).format_with(",", |i, f| { f(&format_args!("${i}")) }),
        );

        let mut db = self.cnx();

        let multi_rows = db.query(
            &format!("SELECT * FROM {} WHERE addr IN {}", &self.table_name, &m),
            &addresses,
        )?;
    ...
    async fn guarded_write(
        ...
        let address_check_query = format!("SELECT word FROM {} WHERE addr = $1", &self.table_name);

        let q = format!(
            "{}",
            (1..=bindings.len() * 2)
                .tuples()
                .format_with(", ", |(i, j), f| { f(&format_args!("(${i}, ${j})")) }),
        );
...
mod tests {
    ...
    fn test_vector_push() {
        ...
        let table = memory.cnx().batch_execute(&format!(
        ...
        assert_eq!(
            block_on(memory.guarded_write((vec![0], None), vec![
                (vec![0], vec![2]),
                (vec![1], vec![1]),
                (vec![2], vec![1])
            ]))
        ...
        assert_eq!(
            block_on(memory.guarded_write((vec![0], Some(vec![2])), vec![
                (vec![0], vec![4]),
                (vec![3], vec![3]),
                (vec![4], vec![3])
            ]))
